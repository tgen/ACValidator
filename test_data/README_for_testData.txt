...
python ACValidator.py -i positive1 -c 8:103312227-103372418 --log-filename Log.txt
...
./ACV_launcher.sh positive1 test_coordinates.txt Log.txt


2) To run after pip install
a) For a single coordinate:
ACValidator -i positive1 -c 8:103312227-103372418 --log-filename Log.txt

b) For all the coordinates in test_coordinates.txt:
for coordinate in `cat test_coordinates.txt`; do ACValidator -i positive1 -c ${coordinate} --log-filename Log.txt; done 
